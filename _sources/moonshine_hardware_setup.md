...
|  |  |  |  | $ |  |  |
```{note}
Estimated total cost for one MoonShine system (excluding the illuminance measuring device) = $~560 (Feb 2023)
```
...
[^4]: To achieve maximum intensity, each LED strip requires a 5V-DC 10A power supply.
...
Connection diagram for the Raspberry Pi, RTC module, and LED strips. The solid color circles (red, white, green) indicate the pins connected to the first array of LED strips for moonlight recreation. The open color circles are connected to the second array of LED strips, for simultaneous recreation of sunlight/twilight. The wire color schematic is for the SK6812, made by BTF-Lighting, Guangdong, China.
```
...
7. Connect additional LED strips to this array from the other end (away from the Raspberry Pi) of the first LED strip. Add an additional DC power supply to each subsequent LED strips.
...
8. Install the RTC module as shown in {numref}`schematic`. Make sure that it goes into the correct location, occupying five of the GPIO pins.
